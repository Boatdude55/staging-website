/**
 * @fileoverview Renderer for {@link app.structor.TopBarAccounts}s.
 *
 * @author attila@google.com (Attila Bodis)
 */

goog.provide('app.structor.TopBarAccountsRenderer');
...
/**
 * Default renderer for {@link app.structor.TopBarAccounts}s.  Each item has the following
 * structure:
 *
 ...
 */
app.structor.TopBarAccountsRenderer = function() {
...
goog.inherits(app.structor.TopBarAccountsRenderer, goog.ui.ControlRenderer);
goog.addSingletonGetter(app.structor.TopBarAccountsRenderer);
 ...
 * @type {string}
 */
app.structor.TopBarAccountsRenderer.CSS_CLASS = goog.getCssName('ui-structor-topbar-accounts');
 ...
 * @private
 */
app.structor.TopBarAccountsRenderer.CompositeCssClassIndex_ = {
  HOVER: 0,
  CHECKBOX: 1,
  CONTENT: 2
};
 ...
 * @param {app.structor.TopBarAccountsRenderer.CompositeCssClassIndex_} index Index for the
 *     CSS class - could be highlight, checkbox or content in usual cases.
 * @return {string} The composite CSS class.
 * @private
 */
app.structor.TopBarAccountsRenderer.prototype.getCompositeCssClass_ = function(index) {
  var result = this.classNameCache_[index];
  if (!result) {
    switch (index) {
      case app.structor.TopBarAccountsRenderer.CompositeCssClassIndex_.HOVER:
        result = goog.getCssName(this.getStructuralCssClass(), 'highlight');
        break;
      case app.structor.TopBarAccountsRenderer.CompositeCssClassIndex_.CHECKBOX:
        result = goog.getCssName(this.getStructuralCssClass(), 'checkbox');
        break;
      case app.structor.TopBarAccountsRenderer.CompositeCssClassIndex_.CONTENT:
...
/** @override */
app.structor.TopBarAccountsRenderer.prototype.getAriaRole = function() {
 ...
 * @override
 */
app.structor.TopBarAccountsRenderer.prototype.createDom = function(item) {
...
/** @override */
app.structor.TopBarAccountsRenderer.prototype.getContentElement = function(element) {
 ...
 * @override
 */
app.structor.TopBarAccountsRenderer.prototype.decorate = function(item, element) {
  ...
  if (goog.dom.classlist.contains(element, goog.getCssName('goog-option'))) {
    (/** @type {app.structor.TopBarAccounts} */ (item)).setCheckable(true);
    this.setCheckable(item, element, true);
  }
  return app.structor.TopBarAccountsRenderer.superClass_.decorate.call(
      this, item, element);
};
 ...
 * @override
 */
app.structor.TopBarAccountsRenderer.prototype.setContent = function(element, content) {
  // Save the checkbox element, if present.
  var contentElement = this.getContentElement(element);
  var checkBoxElement =
      this.hasCheckBoxStructure(element) ? contentElement.firstChild : null;
  app.structor.TopBarAccountsRenderer.superClass_.setContent.call(this, element, content);
 ...
 * @protected
 */
app.structor.TopBarAccountsRenderer.prototype.hasContentStructure = function(element) {
  var child = goog.dom.getFirstElementChild(element);
  var contentClassName = this.getCompositeCssClass_(
      app.structor.TopBarAccountsRenderer.CompositeCssClassIndex_.CONTENT);
 ...
 * @protected
 */
app.structor.TopBarAccountsRenderer.prototype.createContent = function(content, dom) {
  var contentClassName = this.getCompositeCssClass_(
      app.structor.TopBarAccountsRenderer.CompositeCssClassIndex_.CONTENT);
 ...
 */
app.structor.TopBarAccountsRenderer.prototype.setSelectable = function(
 ...
 */
app.structor.TopBarAccountsRenderer.prototype.setCheckable = function(
 ...
 * @protected
 */
app.structor.TopBarAccountsRenderer.prototype.hasCheckBoxStructure = function(element) {
  var contentElement = this.getContentElement(element);
  if (contentElement) {
    var child = contentElement.firstChild;
    var checkboxClassName = this.getCompositeCssClass_(
        app.structor.TopBarAccountsRenderer.CompositeCssClassIndex_.CHECKBOX);
 ...
 * @protected
 */
app.structor.TopBarAccountsRenderer.prototype.setEnableCheckBoxStructure = function(
    item, element, enable) {
 ...
 * @override
 */
app.structor.TopBarAccountsRenderer.prototype.getClassForState = function(state) {
  switch (state) {
    case goog.ui.Component.State.HOVER:
      // We use 'highlight' as the suffix, for backwards compatibility.
      return this.getCompositeCssClass_(
          app.structor.TopBarAccountsRenderer.CompositeCssClassIndex_.HOVER);
    ...
    default:
      return app.structor.TopBarAccountsRenderer.superClass_.getClassForState.call(
          this, state);
  }
};
 ...
 * @override
 */
app.structor.TopBarAccountsRenderer.prototype.getStateFromClass = function(className) {
  ...
  switch (className) {
    ...
    default:
      return app.structor.TopBarAccountsRenderer.superClass_.getStateFromClass.call(
          this, className);
  }
};
...
/** @override */
app.structor.TopBarAccountsRenderer.prototype.getCssClass = function() {
  return app.structor.TopBarAccountsRenderer.CSS_CLASS;
};
